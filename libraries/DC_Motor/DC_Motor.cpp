...
it can provide soft start to motor - means the 
speed of motor will gradually increase from minimum to the desire level

also it can provide smooth stop to motor - means
 the speed of motor will gradually decrease from the current speed to minimum

just one has to select arduino pins for motor,
then then start rotating motor using given library functions
...
DC_Motor::DC_Motor(int pin1,int pin2)   // this is a constructor
{
   ...
   // this will create an instance of DC_Motor
     ...
     pinMode(pin2,OUTPUT);                                            
  // in the arduino sketch with motor driver pins
...
DC_Motor::DC_Motor(int pin1,int pin2,int speed_flag)  
...
{                                                   
 ...
 // in the arduino sketch with motor driver pins with analog output only
...
/////////////////////////// this function will start rotating motor in either direction //////////////////////////////  
 ...
 void DC_Motor::start_motor(int dir)                        
  ...
  {      
    ...
    if(dir==1)                                          
    //if direction is set to 1 it will rotate forward
  ...
  // else for 0 it will rotate reverse
